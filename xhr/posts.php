...
// Check if all images are deleted and handle accordingly
if (!empty($_POST['post_id']) && isset($_POST['delete_all_images']) && $_POST['delete_all_images'] == 'yes') {
    // Delete all images related to the post
    Wo_DeletePostImages($_POST['post_id']);
    
    // Reset the image state for the post in the database
    $db->where('id', $_POST['post_id'])->update(T_POSTS, array('postFile' => ''));
}

// Proceed with uploading new images as usual, regardless of the previous image state
if (!empty($_FILES['new_images'])) {
    $image_urls = array();
    foreach ($_FILES['new_images']['tmp_name'] as $key => $tmp_name) {
        // Process each uploaded image (assuming Wo_UploadImage handles the image upload)
        $image_url = Wo_UploadImage(array(
            'file' => $_FILES['new_images']['tmp_name'][$key],
            'name' => $_FILES['new_images']['name'][$key],
            'size' => $_FILES['new_images']['size'][$key]
        ));
        
        // If upload is successful, add the image URL to the array
        if (!empty($image_url)) {
            $image_urls[] = $image_url;
        }
    }
    
    // Update the post with the new images if any were successfully uploaded
    if (!empty($image_urls)) {
        $db->where('id', $_POST['post_id'])->update(T_POSTS, array('postFile' => implode(',', $image_urls)));
    }
}
$image_urls = array();
            $page_body  = '';
