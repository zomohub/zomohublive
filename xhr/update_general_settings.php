    ...
    if (isset($_POST) && Wo_CheckSession($hash_id) === true) {
        if (empty($_POST['username']) OR empty($_POST['email'])) {
            ...
            if (!empty($Userdata['user_id'])) {
                ...
                // print_r($_POST);
                // if($_POST['user_info_show'])
                // {
                //     $user_info_show_array = array();
                //     foreach($_POST['user_info_show'] as $key => $user_info_show)
                //         {
                //             $user_info_show_array[($key)] = Wo_Secure($user_info_show); 
                //         }
                //     // $user_info_show = json_encode($_POST['user_info_show']);
             
                //     echo $user_info_show_json = json_encode($user_info_show_array);
                //     print_r($_POST['user_info_show']);
                // }
                //$keyword           = Wo_Secure($_POST['user_info_show']);
                //print_r($keyword);
                // echo json_encode($_POST['user_info_show']);
                if(isset($_POST['user_info_show'])){
                    $user_info_show = Wo_Secure(implode(",", $_POST['user_info_show']));
                }
                else
                {
                    $user_info_show = '';
