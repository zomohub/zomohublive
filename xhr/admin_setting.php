...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'add_new_poll') {
        if (Wo_CheckSession($hash_id) === true && !empty($_POST['poll_title'])) {
            ...
            $poll_id = $db->insert(T_ADMIN_POLLS, $poll_data);

            foreach($_POST['poll_answer'] as $poll_answer)
            {
                $poll_ans_date = array(
                    'poll_id' => $poll_id,
                    'pollAnswer' => Wo_Secure($poll_answer),
                    'time' => time()
                );
            
                $id = $db->insert(T_POLLS_ANS, $poll_ans_date);
            }
    ...
    if ($s == 'edit_poll') {
        if (Wo_CheckSession($hash_id) === true && !empty($_POST['poll_id']) && !empty($_POST['poll_title'])) {
            ...
            $poll_exists = $db->where('id', $poll_id)->getValue(T_ADMIN_POLLS, 'count(*)');
            ...
            if ($poll_exists > 0) {
                ...
                // Update the poll in T_ADMIN_POLLS
                $db->where('id', $poll_id);
                $db->update(T_ADMIN_POLLS, $poll_data);
    
                // Fetch existing poll answers from the database
                $existing_answers = $db->where('poll_id', $poll_id)->get(T_POLLS_ANS, null, array('id', 'pollAnswer'));
    
                // Create an array of existing answer IDs
                $existing_ids = array_column($existing_answers, 'id');
    
                // Prepare an array of the IDs from the POST request
                $posted_ids = array_keys($_POST['poll_answer']);
    
                // Step 1 - Update existing answers
                foreach ($_POST['poll_answer'] as $answer_id => $poll_answer) {
                    if (in_array($answer_id, $existing_ids)) {
                        // Update answer if ID exists in the database
                        $answer_data = array(
                            'pollAnswer' => Wo_Secure($poll_answer),
                            'time' => time()
                        );
                        $db->where('id', $answer_id);
                        $db->update(T_POLLS_ANS, $answer_data);
                    } else {
                        // Step 3 - Add new answer if ID does not exist
                        $new_answer_data = array(
                            'poll_id' => $poll_id,
                            'pollAnswer' => Wo_Secure($poll_answer),
                            'time' => time()
                        );
                        $db->insert(T_POLLS_ANS, $new_answer_data);
                    }
                }
    
                // Step 2 - Delete answers that are no longer present in the POST data
                ...
                if (!empty($ids_to_delete)) {
                    ...
                    {
                        $query_two = "DELETE FROM " . T_POLLS_ANS . " WHERE `id` = {$id}";
                ...
                $data['status'] = 200;
                $data['message'] = 'Poll updated successfully';
            } else {
                $data = array(
                    'status' => 404,
    ...
    if ($s == 'delete_poll') {
        ...
        if (!empty($_GET['id'])) {
            ...
            $query_one = "DELETE FROM " . T_ADMIN_POLLS . " WHERE `id` = {$_GET['id']}";
            $sql   = mysqli_query($sqlConnect, $query_one);

            $query_two = "DELETE FROM " . T_POLLS_ANS . " WHERE `poll_id` = {$_GET['id']}";
            $sql   = mysqli_query($sqlConnect, $query_two);

            $query_three = "DELETE FROM " . T_POLLS_ANS_COUNT . " WHERE `poll_id` = {$_GET['id']}";
            $sql   = mysqli_query($sqlConnect, $query_three);
