...
if (empty($error_code)) {
    ...
    if (empty($recipient_data)) {
		...
		if (!empty($data['user_data'])) {
			...
			$visibility_flags = !empty($_POST['info_visibility']) ? explode(',', $_POST['info_visibility']) : [];
		
			// Map visibility flags to recipient data keys
			$visibility_map = [
				'show_gender' => 'gender',
				'show_dob'    => 'birthday',
				'show_email'  => 'email',
			...
			// Hide keys not included in visibility flags
			foreach ($visibility_map as $flag => $key) {
				if (!in_array($flag, $visibility_flags) && isset($recipient_data[$key])) {
					unset($recipient_data[$key]);
				}
			}
		...
		if (!empty($data['family'])) {
			...
			if (!empty($data['images'])) {
				$response_data['images'] = Wo_GetPosts(array(
					'filter_by' => 'photos',
					'publisher_id' => $recipient_data['user_id'],
					'limit' => 50
				));
			}

			if (!empty($data['videos'])) {
				$response_data['videos'] = Wo_GetPosts(array(
					'filter_by' => 'video',
					'publisher_id' => $recipient_data['user_id'],
					'limit' => 50
				));
			...
			if (!empty($data['reels'])) {
				$response_data['reels'] = Wo_GetPosts(array(
					'is_reel' => 'only',
					'publisher_id' => $recipient_data['user_id'],
					'limit' => 50
				));
