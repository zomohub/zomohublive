...
function Wo_UpdateUserData($user_id, $update_data, $unverify = false)
{
    ...
    foreach ($update_data as $field => $data) {
        $filter = ['first_name', 'last_name', 'about', 'user_info_show'];
...
function Wo_GetAdminPolls() {
    global $sqlConnect;
    $data = array();

    // Fetch all polls
    $query_one = "SELECT * FROM " . T_ADMIN_POLLS . " ORDER BY `id` DESC";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);

    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $poll_id = $fetched_data['id'];

            // Fetch poll answers based on poll_id
            $query_two = "SELECT * FROM " . T_POLLS_ANS . " WHERE `poll_id` = $poll_id";
            $sql_query_two = mysqli_query($sqlConnect, $query_two);
            $poll_answers = array();
            $total_poll_count = 0;

            if (mysqli_num_rows($sql_query_two)) {
                while ($answer_data = mysqli_fetch_assoc($sql_query_two)) {
                    $poll_ans_id = $answer_data['id'];

                    // Count the number of votes for each poll answer (poll_ans_id)
                    $query_three = "SELECT COUNT(*) AS answerCount FROM " . T_POLLS_ANS_COUNT . " WHERE `poll_ans_id` = $poll_ans_id";
                    $sql_query_three = mysqli_query($sqlConnect, $query_three);
                    $answer_count = 0;

                    if (mysqli_num_rows($sql_query_three)) {
                        $count_data = mysqli_fetch_assoc($sql_query_three);
                        $answer_count = $count_data['answerCount'];
                    }

                    // Add the answer count to the answer data
                    $answer_data['count'] = $answer_count;

                    // Update total poll count (sum of all answer counts for this poll)
                    $total_poll_count += $answer_count;

                    // Store the answer data with the count
                    $poll_answers[] = $answer_data;
                }
            }

            // Add the total poll count to the poll data
            $fetched_data['total_count'] = $total_poll_count;

            // Add poll answers to the poll data
            $fetched_data['answers'] = $poll_answers;

            // Store the poll with answers and their counts
            $data[] = $fetched_data;
        }
    }
...
function Wo_GetAdminPoll($id) {
    ...
    $data      = array();
    $poll_id   = Wo_Secure($id);
    
    // Fetch poll data
    $query_one = "SELECT * FROM " . T_ADMIN_POLLS . " WHERE `id` = '{$poll_id}'";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    
    if (mysqli_num_rows($sql_query_one)) {
        $fetched_data = mysqli_fetch_assoc($sql_query_one);

        // Fetch poll answers from T_POLLS_ANS based on poll_id
        $query_two = "SELECT * FROM " . T_POLLS_ANS . " WHERE `poll_id` = '{$poll_id}'";
        $sql_query_two = mysqli_query($sqlConnect, $query_two);
        $poll_answers = array();
...
error_reporting(E_ALL);
    ...
    if (mysqli_num_rows($sql_query)) {
        while ($poll = mysqli_fetch_assoc($sql_query)) {
            ...
            $poll_id = $poll['id'];
            $poll['answers'] = Wo_GetPollAnswers($poll_id);
            $data[] = $poll;
        }
    }
...
function Wo_GetPollAnswers($poll_id) {
    global $sqlConnect;

    $answers = array();
    $total_votes = 0; // Initialize total poll votes

    // Query to get all answers for the given poll_id
    $query = "SELECT * FROM " . T_POLLS_ANS . " WHERE `poll_id` = '{$poll_id}'";
    $sql_query = mysqli_query($sqlConnect, $query);

    if (mysqli_num_rows($sql_query)) {
        while ($answer = mysqli_fetch_assoc($sql_query)) {
            $poll_ans_id = $answer['id']; // Get poll_ans_id for each answer

            // Query to count the number of votes for the specific poll answer
            $count_query = "SELECT COUNT(*) AS vote_count FROM " . T_POLLS_ANS_COUNT . " WHERE `poll_ans_id` = '{$poll_ans_id}'";
            $count_sql_query = mysqli_query($sqlConnect, $count_query);
            $vote_count = 0;

            if (mysqli_num_rows($count_sql_query)) {
                $count_data = mysqli_fetch_assoc($count_sql_query);
                $vote_count = (int)$count_data['vote_count']; // Get the number of votes for this answer
            }

            // Add the vote count to the answer data
    ...
    return array(
        'answers' => $answers,
        'total_votes' => $total_votes // Return the total number of votes for the poll
    );
}


function Wo_IsAdminPollVoted($poll_id, $user_id) {
    global $sqlConnect;

    // Ensure poll_id and user_id are not empty
    if (empty($poll_id) || empty($user_id)) {
        return false;
    }

    // Secure the input to prevent SQL injection
    $poll_id = Wo_Secure($poll_id);
    $user_id = Wo_Secure($user_id);

    // Query to check if the user has already voted in this poll
    $query = "SELECT COUNT(*) AS voteCount 
              FROM " . T_POLLS_ANS_COUNT . " 
              WHERE `poll_id` = '{$poll_id}' AND `user_id` = '{$user_id}'";

    $sql_query = mysqli_query($sqlConnect, $query);
    if (mysqli_num_rows($sql_query)) {
        $result = mysqli_fetch_assoc($sql_query);
        // If the user has at least one vote, return true
        if ($result['voteCount'] > 0) {
            return true;
        }
    }

    // If no vote found, return false
    return false;
}


function Wo_adminPollVoteUp($poll_ans_id) {
    global $sqlConnect, $wo;
    ...
    $poll_ans_id = Wo_Secure($poll_ans_id);
    $user_id = Wo_Secure($user_id);

    // Fetch poll_id from the answer
    $pollQuery = "SELECT poll_id FROM " . T_POLLS_ANS . " WHERE id = '{$poll_ans_id}'";
    $pollResult = mysqli_query($sqlConnect, $pollQuery);
    if (mysqli_num_rows($pollResult)) {
        $pollData = mysqli_fetch_assoc($pollResult);
        $poll_id = $pollData['poll_id'];
    } else {
        return false; // Invalid poll answer
    }

    // Check if the user has already voted on this }
    if (Wo_IsAdminPollVoted($poll_id, $user_id)) {
        return false; // User already voted
    }

    // Insert vote into T_POLLS_ANS_COUNT
    $voteData = array(
        'poll_id' => $poll_id,
        'poll_ans_id' => $poll_ans_id,
        'user_id' => $user_id,
        'time' => time()
    );
    $vote_id = $db->insert(T_POLLS_ANS_COUNT, $voteData);
...
function Wo_GetPollIDFromAnsID($poll_ans_id) {
    global $sqlConnect;

    // Secure the input
    $poll_ans_id = Wo_Secure($poll_ans_id);

    // Query to fetch the poll_id from the T_POLLS_ANS table based on poll_ans_id
    $query = "SELECT poll_id FROM " . T_POLLS_ANS . " WHERE id = '{$poll_ans_id}' LIMIT 1";
    $sql_query = mysqli_query($sqlConnect, $query);

    if (mysqli_num_rows($sql_query)) {
        $result = mysqli_fetch_assoc($sql_query);
        return $result['poll_id'];  // Return the poll_id
    }

    return false; // Return false if no matching poll_id is found
}
