...
function Wo_EditComment_btn(comment_id) {
...
function Wo_VoteUp(id) {
  ...
  $.get(Wo_Ajax_Requests_File(), { f: 'vote_up', id: id }, function (data) {
    if (data.status == 200) {
      ...
      $('#poll-alert-msg-' + $post_id).text(data.text)
        .show()
        .delay(3000)
        .fadeOut();
    }
...
function Wo_adminPollVoteUp(id) {
  var $vote_con = $('#admin-poll-option-' + id);
  var $poll_id = $vote_con.attr('data-admin-poll-id');

  var $total_count = $('#admin-poll-total-count-' + $poll_id);
  ...
  // }
  console.log($poll_id);
  $.get(Wo_Ajax_Requests_File(), { f: 'admin_poll_vote_up', id: id }, function (data) {
    if (data.status == 200) {

      var $spanElement = $vote_con.find('span'); // Find the span element inside the li
      var currentCount = parseInt($spanElement.text());

      if (!isNaN(currentCount)) { // Check if the current count is a valid number
        var updatedCount = currentCount + 1; // Increment the count by 1
        $spanElement.text(updatedCount); // Update the span content with the new count
      }

      var $spanElement = $total_count.find('span'); // Find the span element inside the li
      var currentCount = parseInt($spanElement.text());

      if (!isNaN(currentCount)) { // Check if the current count is a valid number
        var updatedCount = currentCount + 1; // Increment the count by 1
        $spanElement.html('<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"><path fill="currentColor" d="M3,22V8H7V22H3M10,22V2H14V22H10M17,22V14H21V22H17Z"></path></svg> ' + updatedCount); // Update the span content with the new count
        $spanElement.show();
      }
      ...
      $('#admin-poll-alert-msg-' + $poll_id).html('<div class="alert alert-voted" role="alert" bis_skin_checked="1">' + data.text + '</div>')
        .show()
        .delay(3000)
        .fadeOut();
